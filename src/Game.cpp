...
{
	//Load font
	if (!this->font.loadFromFile("fonts/04B_03.TTF"))
	...
	this->pointText.setPosition(680.f, 5.f);
...
void Game::initPlayer()
{
	...
	this->player->setPosition(400, 600);
}
...
{
	//Move player
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::A)) { this->player->move(-1.f, 0.f); }
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::D)) { this->player->move(1.f, 0.f); }
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::W)) { this->player->move(0.f, -1.f); }
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::S)) { this->player->move(0.f, 1.f); }
...
{
	//Spawning
	this->spawnTimer += 0.5f + static_cast<float>(this->points/100);
	if (this->spawnTimer >= this->spawnTimerMax)
	{
		this->enemies.push_back(new Enemy(rand() % this->window->getSize().x-40.f + 20.f, -100.f));
		this->spawnTimer = 0.f;
	}
